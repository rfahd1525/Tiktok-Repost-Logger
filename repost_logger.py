...
class TikTokRepostLogger:
    ...
    async def initialize_browser(self):
        ...
        try:
            ...
            # Create browser context with stealth settings to avoid bot detection
    ...
    async def fetch_reposts(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            try:
                await self.page.wait_for_selector('[data-e2e="user-avatar"], [data-e2e="user-title"], h1, h2', timeout=10000)
            ...
            repost_tab_selectors = [
                # Text-based selectors (most reliable for changing structures)
                'p:has-text("Reposts")',
                ...
                'a:has-text("Reposts")',
                # Generic fallbacks
                'div:has-text("Reposts"):not(:has(div:has-text("Reposts")))',
            ]
            ...
            # If tab not found, try reloading the page once before failing
            if not repost_tab_found:
                print("Reposts tab not found on first attempt, reloading page...")
                ...
                for selector in repost_tab_selectors:
                    try:
                        ...
                        if repost_tab:
                            await repost_tab.click()
                            repost_tab_found = True
                            print("Clicked on Reposts tab (after reload)")
                            await asyncio.sleep(2)
                            break
                    except PlaywrightTimeout:
                        continue
